...
function editar(novoStatus, idEmpresa) {
    ...
    var instrucaoSql = `
        UPDATE empresa e INNER JOIN funcionario f on f.empresa_id = e.id SET e.statusAcesso = '${novoStatus}', f.perfilAtivo = ${novoStatus == "2" ? false : true} WHERE e.id = ${idEmpresa};
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
